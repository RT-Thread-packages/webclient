...
#define rt_strdup   webclient_strdup
        ...
        const char *url, char** request)
{
    ...
    *request = (char*) url;
    ...
    /* get host addr ok. */
    {
        char *host_addr_new = malloc(host_addr_len+1);
    ...
    if (is_domain)
    {
        ...
        hptr = lwip_gethostbyname(session->host);
        if (hptr == 0)
        {
        ...
        const char *header, size_t header_sz)
{
    ...
    if (header == RT_NULL)
    {
        header_buffer = malloc (WEBCLIENT_HEADER_BUFSZ);
...
int webclient_handle_response(struct webclient_session* session)
{
    ...
    mimeBuffer = (char*)malloc(BUF_SZ + 1);
    if (!mimeBuffer)
        return -1;
            ...
            && strcmp(session->transfer_encoding, "chunked") == 0)
    {
        ...
        webclient_read_line(session->socket, mimeBuffer, BUF_SZ);
...
int webclient_connect(struct webclient_session* session, const char *URI)
{
    ...
    lwip_setsockopt(socket_handle, SOL_SOCKET, SO_RCVTIMEO, (void*) &timeout,
            sizeof(timeout));
    lwip_setsockopt(socket_handle, SOL_SOCKET, SO_SNDTIMEO, (void*) &timeout,
            sizeof(timeout));

    if (lwip_connect(socket_handle, (struct sockaddr *) &server,
...
struct webclient_session* webclient_open(const char* URI)
{
    ...
    /* create session */
    session = (struct webclient_session*) malloc(sizeof(struct webclient_session));
    if (session == RT_NULL)
        return RT_NULL;
...
struct webclient_session* webclient_open_position(const char* URI, int position)
{
    ...
    range_header = (char*)malloc (WEBCLIENT_HEADER_BUFSZ);
...
int webclient_set_timeout(struct webclient_session* session, int millisecond)
{
    ...
    /* set recv timeout option */
    lwip_setsockopt(session->socket, SOL_SOCKET, SO_RCVTIMEO,
            (void*) &millisecond, sizeof(millisecond));
    lwip_setsockopt(session->socket, SOL_SOCKET, SO_SNDTIMEO,
            (void*) &millisecond, sizeof(millisecond));
        ...
        size_t length)
{
    ...
    do
    {
        rt_kprintf("session->socket : %d , totalRead : %d\n",session->socket, totalRead);
        bytesRead = lwip_recv(session->socket, buffer + totalRead, left, 0);
		rt_kprintf("bytesRead : %d\n", bytesRead);
        if (bytesRead <= 0)
        {
            if (totalRead)
                break;
...
int webclient_close(struct webclient_session* session)
{
    ...
    if (session->socket >= 0)
        lwip_close(session->socket);
    free(session->transfer_encoding);
    free(session->content_type);
    free(session->last_modified);
    free(session->host);
    ...
    *response = NULL; /* initialize response */
    ...
    if (session->content_length < 0) /* not content length field kind */
    {
        ...
        while (1)
        {
            ...
            new_resp = realloc(response_buf, result_sz + 1);
            if(!new_resp)
            {
    ...
    else
    {
        ...
        response_buf = malloc(result_sz + 1);
        ...
        const char* form_data)
{
    ...
    header = (char*) malloc(WEBCLIENT_HEADER_BUFSZ);
    if (header == RT_NULL)
    {
        rc = -WEBCLIENT_NOMEM;
        goto __exit;
    ...
    buffer_ptr += rt_snprintf((char*) buffer_ptr,
            BUF_SIZE - (buffer_ptr - buffer), "--%s\r\n", boundary);
    buffer_ptr += rt_snprintf((char*) buffer_ptr,
            BUF_SIZE - (buffer_ptr - buffer),
